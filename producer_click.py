...
def data():
    ...
    res = client.execute(
        "select wh_id, office_id from dictionary.Warehouse limit 100"
    )
...
def send_message(data):
    try:
        ...
        producer.produce("warehouse", data.encode("utf-8"), callback=delivery_report)
...
if __name__ == "__main__":
    ...
    for i in range(len(res)):
        result = res[i]
        pp = pd.DataFrame([result], columns=["wh_id", "office_id"])
